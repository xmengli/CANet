...
class ResNet(nn.Module):
    def __init__(self, block, layers,  network_type, num_classes, att_type=None, joint=None):
        self.inplanes = 64
        ...
        if self.joint == "BAM":
            self.branch_bam1 = BAM(512*block.expansion)
            self.branch_bam2 = BAM(512*block.expansion)
        elif self.joint == "CBAM":
            self.dropout = nn.Dropout(0.3)
            self.branch_bam1 = CBAM(512*block.expansion)
            self.branch_bam2 = CBAM(512*block.expansion)

            # self.classifier1_1 = nn.Linear(512 * block.expansion, 256)
            # self.classifier2_1 = nn.Linear(512 * block.expansion, 256)
            # self.classifier1_1 = nn.Linear(512*block.expansion, 256)
            ...
            self.classifer2 = nn.Linear(512 * block.expansion, 3)
        else:
            self.classifer1 = nn.Linear(512 * block.expansion, num_classes)
            self.classifer2 = nn.Linear(512 * block.expansion, 3)


        init.kaiming_normal_(self.classifer1.weight)
        init.kaiming_normal_(self.classifer2.weight)
    ...
    def forward(self, x):
        ...
        x1 = self.classifer1(x1)
        x2 = self.classifer2(x2)
        # x1 = self.classifier1_3(self.classifier1_2(self.classifier1_1(x1)))
        # x2 = self.classifier2_3(self.classifier2_2(self.classifier2_1(x2)))
